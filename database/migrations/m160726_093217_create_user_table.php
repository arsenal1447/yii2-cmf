<?php

use yii\db\Migration;
use yii\db\mysql\Schema;

/**
 * Handles the creation for table `{{%user}}`.
 */
class m160726_093217_create_user_table extends Migration
{
    /**
     * @inheritdoc
     */
    public function up()
    {
        // user
        $this->createTable('{{%user}}', [
            'id' => Schema::TYPE_PK,
            'username' => $this->string(255)->notNull()->unique(),
            'auth_key' => Schema::TYPE_STRING . "(32)  NOT NULL DEFAULT 0 ",
            'password_hash' => Schema::TYPE_STRING . "(255)  NOT NULL DEFAULT 0 ",
            'password_reset_token' => Schema::TYPE_STRING . "(255) NULL",
            'email' => Schema::TYPE_STRING . "(255)  NOT NULL DEFAULT 0 ",
            'status' => Schema::TYPE_SMALLINT . "(6) NOT NULL DEFAULT '10'",
            'created_at' => Schema::TYPE_INTEGER . "(11)  NOT NULL DEFAULT 0 ",
            'updated_at' => Schema::TYPE_INTEGER . "(11)  NOT NULL DEFAULT 0 ",
            'is_admin' => Schema::TYPE_BOOLEAN . " NOT NULL DEFAULT '0'",
            'login_at' => Schema::TYPE_INTEGER . "(11) NULL",
            'blocked_at' => $this->integer()->null(),
            'confirmed_at' => $this->integer()->null()
        ]);
        // profile
        $this->createTable('{{%profile}}', [
            'user_id' => Schema::TYPE_PK,
            'money' => Schema::TYPE_INTEGER . "(11) NOT NULL DEFAULT 0",
            'signature' => Schema::TYPE_STRING . "(100) NOT NULL DEFAULT ''",
            'avatar' => Schema::TYPE_STRING . "(255) NOT NULL DEFAULT ''",
            'gender' => Schema::TYPE_BOOLEAN . " NOT NULL DEFAULT '0'",
            'qq' => $this->string(20),
            'phone' => $this->string(20),
            'province' => $this->smallInteger(4),
            'city' => $this->smallInteger(4),
            'area' => $this->smallInteger(4),
            'locale' => Schema::TYPE_STRING . "(32) NOT NULL DEFAULT 'zh-CN'",
            'created_at' => Schema::TYPE_INTEGER . "(10)  NOT NULL DEFAULT 0 ",
            'updated_at' => Schema::TYPE_INTEGER . "(10)  NOT NULL DEFAULT 0 "
        ]);

        $this->insert('{{%user}}', [
            'username' => 'hehe',
            'auth_key' => '1lQl4TG6sYlyWRqXZEWL0ZhQkPATVnMs',
            'password_hash' => '$2y$13$lYlhIcBcs6jBr7yTd6YrWueckcs.Cvx70juIHs6wEfjtUwnA318VW',
            'email' => 'hehe@xxx.com',
            'status' => 10,
            'created_at' => 1441766741,
            'updated_at' => 1441766741,
            'login_at' => '1441766741',
            'confirmed_at' => '1441766741'
        ]);
        $this->insert('{{%profile}}', [
            'user_id' => 1,
            'locale' => 'zh-CN'
        ]);
    }

    /**
     * @inheritdoc
     */
    public function down()
    {
        $this->dropTable('{{%user}}');
        $this->dropTable('{{%profile}}');
    }
}
